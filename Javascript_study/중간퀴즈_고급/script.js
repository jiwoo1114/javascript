...
const kor = student.subject.kor
const math = student.subject.math
const eng = student.subject.eng
const science = student.subject.science

scores=[kor,math,eng,science]
...
const avarrage = ((scores) => {
   let sum = 0
   for (score of scores) {
      sum += score
   }

   const avg = (sum / 5).toFixed(0)
...
console.log(`3번 문제 정답: ${avarrage(scores)}점`)
...
const cellphone = student.phone.replaceAll('-', '')
 

console.log(`8번 문제 정답: ${cellphone}`)
...
const reg_date = student.reg_date //객체에서 날짜 출력
const date_arrs = reg_date.split('/') // '2024/01/14'

const date = new Date(date_arrs[0], date_arrs[1] - 1, date_arrs[2])
date.setDate(date.getDate() + 20) //date 함수 날짜 변경 

console.log(`9번 문제 정답: ${date}`)



//console.log(`9번 문제 정답: ${date.getFullYear()} ${date.getMonth()+2} ${date.getDate()-11} ${date.getHours()}-${date.getTime()}`)
...
const result = employees.filter((employ) => {
   //서울이 포함 안 되어 있을 떄 
   if (employ.addr.indexOf('서울') === -1) {
      console.log('서울에 살지 않습니다')
      return false 
      //서울이 포함되어 있을 때. 
   } else { console.log('서울에 삽니다') }
   return true
})/*.forEach((addr) => {
   console.log(`10번 문제 정답:${addr.name},${addr.addr}`)
}
)*/
...
const result2 = employees.map((employ) => {

   employ.dept = '개발부'
  return (`이름:${employ.name},주소:${employ.addr},부서:${employ.dept}`)
